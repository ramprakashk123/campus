...
const StudentDiscussions = () => {
  ...
  useEffect(() => {
    API.get('/courses/enrolled').then((r) => { setCourses(r.data); if (r.data.length > 0) setSelectedCourse(r.data[0]._id); }).catch(() => {}).finally(() => setLoading(false));
  }, []);
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div><h1 className="text-2xl font-bold text-white">Discussion Forum</h1><p className="text-slate-400 mt-1">Connect with classmates and faculty</p></div>
            ...
            <div key={d._id} className={`glass-card p-5 ${d.isPinned ? 'border-indigo-500/30' : ''}`}>
              <div className="flex items-start justify-between">
                <div className="flex items-start gap-3">
                  <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-blue-500 to-cyan-600 flex items-center justify-center text-white text-xs font-bold flex-shrink-0">{d.author?.name?.charAt(0)}</div>
                  <div>
                ...
                <div className="mt-4 ml-11 space-y-3 border-l-2 border-white/5 pl-4">
                  {d.replies.map((r, i) => (
                    <div key={r._id || i} className="flex items-start gap-2">
                      ...
                      <div><p className="text-xs text-slate-500">{r.author?.name} • {new Date(r.createdAt).toLocaleDateString()}</p><p className="text-sm text-slate-300">{r.body}</p></div>
                    </div>
                  ))}
                </div>
              )}
